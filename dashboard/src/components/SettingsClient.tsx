...
export default function SettingsClient() {
  ...
  const [verifiedInSession, setVerifiedInSession] = useState<
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0b0e14] text-slate-400 font-mono">
        <div className="animate-pulse flex items-center gap-2">
          ...
          <span>INITIALIZING ARGUS_VAULT...</span>
        </div>
      </div>
    );
    ...
    >
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <aside
            className="space-y-6"
            aria-label="Provider selection and security"
          >
            <div>
              <label
              ...
              >
                <div className="flex-1">
                  <div className="flex justify-between mb-1">
                    <span
                      id={`provider-name-${p.id}`}
                      className="text-[12px] text-slate-400"
                    >
                      {p.name}
                    </span>
                    {hasStoredKey[p.id] && (
                      <span className="text-[9px] text-green-500 uppercase font-bold">
                        Vaulted
                      </span>
                    )}
                  </div>
                ...
                <button
                  ...
                  title={
                    keys[p.id] !== "" ? "Store in Vault to enable testing" : ""
                  }
                >
                  {testing === p.id ? (
                    <RefreshCw
              ...
              >
                {Object.values(keys).some((k) => k !== "") && (
                  <span className="text-[8px] text-blue-400 uppercase font-black animate-pulse flex items-center gap-1">
                    <ShieldCheck size={10} /> Pending Changes: Store in Vault to
                    Test
                  </span>
                )}
                {!canSave && saving === "idle" && (
