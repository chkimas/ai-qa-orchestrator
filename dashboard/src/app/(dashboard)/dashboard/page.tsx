...
export default async function Home() {
  ...
  if (error) {
    console.error('[ARGUS DB ERROR]:', error.message)
    return (
      <main className="p-8 mx-auto min-h-screen bg-slate-950 text-slate-200">
        <div className="p-8 text-center space-y-4">
          <ShieldAlert className="w-12 h-12 text-red-500 mx-auto" />
          <p className="text-red-400 text-sm">Failed to load missions. Please refresh.</p>
        </div>
      </main>
    )
  }
          ...
          {runData.length === 0 ? (
            <div className="p-20 text-center space-y-4">
              ...
              <p className="text-slate-500 text-sm uppercase tracking-wide font-bold">
                No active missions. Deploy your first neural node.
              </p>
            </div>
          ) : (
